...
def video_to_pic(fileName: str,out_img: list[np.ndarray]):
    subprocess.run(["ffmpeg", "-i", f"./assets/video/{fileName}", "-vf", "fps=25", f"./assets/img/output_%04d.png", ])
    for i in range(1, 200):     #根据预期最大图片数量调整范围
        img_path = f"./assets/img/output_{i:04d}.png"
        if os.path.exists(img_path):
            out_img.append(img_path)
        else:
            break
