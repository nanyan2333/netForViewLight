...
void IdentifyQRcode(Mat img, vector<Mat>& QRcodes) {
	...
	Point2f P2[4]{
		Point2f(35, 35), Point2f(1405, 35), Point2f(35, 1405), Point2f(1375, 1375)
	};

	Mat dstImg;
	Mat matrix = getPerspectiveTransform(P1, P2);
	warpPerspective(midImg, dstImg, matrix, Size(1440, 1440), 1, 0, Scalar(255, 255, 255));
	QRcodes.push_back(dstImg);
	//imwrite(to_string(QRcodes.size()) + ".jpg", dstImg);
}#include "identifyQRcode.h"
